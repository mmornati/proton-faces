...
| `WEBP_METHOD` | `4` | WebP encoder effort for thumbnails/video posters. `6` (Pillow default) is the slowest; `4` is near-identical size at a fraction of the CPU (issue #101). |
...
| `TOTP_ENCRYPTION_KEY` | _(falls back to `SIGNING_SECRET`)_ | Root key for the AES-GCM encryption of stored TOTP secrets. Set it separately from `SIGNING_SECRET` so the URL-signing secret can be rotated freely; rotating this value makes every enrolled 2FA secret undecryptable (recover with `python main.py --disable-2fa <user>`). |
